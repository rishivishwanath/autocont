...
async def avatar_video_pipeline(user_id,mode=0, text=None,audio_url="output/speech.mp3",title="N/A",description="N/A",avatar_id="de90ffeec028414a90ad2d954dc85b41", voice_id="74f0f8d1b27147c4aa9c65f690a3ead3"):
    if mode == 0:
        result=await generate_heygen_video(a=0, text=text,avatar_id=avatar_id, voice_id=voice_id)
    elif mode == 1:
    ...
    await upload_video(output_file, title=title, description=description,user_id=user_id)
    await asyncio.to_thread(cleanup_temp_files(output_file, audio_temp if mode == 1 else None))
